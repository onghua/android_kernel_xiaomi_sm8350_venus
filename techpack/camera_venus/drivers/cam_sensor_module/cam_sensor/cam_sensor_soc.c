...
static bool cam_sensor_venus_get_topology(uint32_t slot,
	enum cci_device_num *cci_num, enum cci_i2c_master_t *master)
{
	if (!cci_num || !master)
		return false;
...
static int32_t cam_sensor_driver_get_dt_data(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (s_ctrl->io_master_info.master_type == CCI_MASTER) {
		...
		if (rc < 0 || s_ctrl->cci_i2c_master >= MASTER_MAX) {
		...
		if (!of_parent || of_property_read_u32(of_parent, "cell-index",
				&s_ctrl->cci_num) < 0)
			/* Set default master 0 */
...
#if defined(CONFIG_MACH_XIAOMI_VENUS)
		{
			enum cci_device_num venus_cci;
			enum cci_i2c_master_t venus_master;

			if (cam_sensor_venus_get_topology(soc_info->index,
					&venus_cci, &venus_master)) {
				if (s_ctrl->cci_num != venus_cci) {
					CAM_WARN(CAM_SENSOR,
						"venus slot %d cci%d -> official cci%d",
						soc_info->index, s_ctrl->cci_num,
						venus_cci);
					s_ctrl->cci_num = venus_cci;
				}
				if (s_ctrl->cci_i2c_master != venus_master) {
					CAM_WARN(CAM_SENSOR,
						"venus slot %d master%d -> official master%d",
						soc_info->index,
						s_ctrl->cci_i2c_master,
						venus_master);
					s_ctrl->cci_i2c_master = venus_master;
				}
			}
		}
#endif
