	...
	if (s_ctrl->soc_info.index < 4) {
		...
		if (rc < 0 &&
			...
			(s_ctrl->sensor_probe_data_type !=
				CAMERA_SENSOR_I2C_TYPE_WORD))) {
			CAM_WARN(CAM_SENSOR,
				"venus slot:%d word id read failed rc:%d, retry userspace type",
...
#if defined(CONFIG_MACH_XIAOMI_VENUS)
	if (!rc && s_ctrl->soc_info.index < 4 &&
		((s_ctrl->sensor_probe_addr_type !=
			CAMERA_SENSOR_I2C_TYPE_WORD) ||
		(s_ctrl->sensor_probe_data_type !=
			CAMERA_SENSOR_I2C_TYPE_WORD))) {
		sensor_id = cam_sensor_id_by_mask(s_ctrl, chipid);
		if (sensor_id != slave_info->sensor_id) {
			uint32_t retry_chipid = 0;
			int retry_rc;

			retry_rc = camera_io_dev_read(
				&(s_ctrl->io_master_info),
				slave_info->sensor_id_reg_addr,
				&retry_chipid,
				s_ctrl->sensor_probe_addr_type,
