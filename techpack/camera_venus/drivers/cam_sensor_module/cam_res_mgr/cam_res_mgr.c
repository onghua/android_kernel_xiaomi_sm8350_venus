	...
	struct device *dev)
{
	...
	const char *pinctrl_gpio_prop = "gpios-shared-pinctrl";
	struct device_node *of_node = NULL;
	struct cam_res_mgr_dt *dt = &cam_res->dt;
	...
	if (dt->num_shared_pctrl_gpio <= 0) {
		pinctrl_gpio_prop = "shared-pinctrl-gpios";
		dt->num_shared_pctrl_gpio =
			of_property_count_u32_elems(of_node, pinctrl_gpio_prop);
	}
	...
	rc = of_property_read_u32_array(of_node, pinctrl_gpio_prop,
		dt->shared_pctrl_gpio, dt->num_shared_pctrl_gpio);
	if (rc) {
		CAM_ERR(CAM_RES, "Get %s array failed.", pinctrl_gpio_prop);
		return -EINVAL;
	}
